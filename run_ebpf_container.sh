...
podman run --name $JOB_ID --tmpfs /run --tmpfs /run/lock -v /sys/fs/cgroup:/sys/fs/cgroup:rw -v /usr/src/kernels/$KERNEL_VER:/usr/src/kernels/$KERNEL_VER:ro -v /lib/modules/$KERNEL_VER:/lib/modules/$KERNEL_VER:ro -v /var/log/abra/$JOB_ID:/var/log/ebpf:rw -p 5801:5801 -p 7681:7681 --cap-add CAP_BPF --cap-add CAP_SYS_ADMIN -d $IMG